...
static bool is_snake(char c) {
  ...
  if (c == '^' || c == '>' || c == '<' || c == 'v' || c == 'W' || c == 'A' || c == 'S' || c == 'D' || c == 'w' || c == 'a' || c == 's' || c== 'd') {//c == is_tail(c) || is_head(c)) {
      return true;
  }
  return false;
}
  ...
  // TODO: Implement this function.
    ...
    for (unsigned int i = 0; i < game -> num_snakes; i++) {
        ...
        if (is_snake(next_step) || next_step == '#') {
            game->board[game -> snakes[i].head_row][game -> snakes[i].head_col] = 'x'; //fixed because no pointer to the board
...
char *read_line(FILE *fp) {
  // TODO: Implement this function.
  // read it by how many characters at a time
  // create a line pointer
  // needs to be large enough so use malloc 
  // keep track of the characters that have been read already
  // use fgets from spec hint
  // need a loop -- while loop for checking there are still characters
  
    if (fp == NULL) {
        return NULL;
    }
  
    char* linereader = (char*)malloc(10 * sizeof(char));
    char reader[11];
    int count = 0;
    
    while (fgets(reader, 11, fp) != NULL) { //not while looping with strcpy, using fgets
        size_t readlen = strlen(reader);

        char *t = realloc(linereader, ((size_t)count + readlen + 1) * sizeof(char)); //from office hours

        linereader = t;

        strcpy(&linereader[count], reader); //from office hours, want to get the address
        count += (int)readlen;

        if (strchr(reader, '\n') != NULL) { // Stop if newline is found
            return linereader;
            //break;
        }
    }

    //linereader[count] = '\0'; // Null-terminate the string
    free(linereader);
    return NULL;
}

            //if (fgets(&linereader[count], 10, fp) == NULL) {
              //  return NULL;
              // don't use fgets use strcpy for htis portion
            // // strcat(linereader, reader); use the address
           // count+=10; //increment to read the next character in the line -- changed to add strlen(reader)

/* Task 5.2 */
game_t *load_board(FILE *fp) {
  // TODO: Implement this function.
    
    //you essentially want to read through all the lines in the file and now assign these lines to the board
    //read_line reads through all the characters of the line and returns it into a linereader object
    // char* read_in_line = read_line(fp); //this is the main idea but it needs to be somewhere else
    
    if (fp == NULL) {
        return NULL;
    }

    //do i need to make a new game struct?

    game_t *game = (game_t *) malloc(sizeof(game_t)); //create a new game struct //it's empty
                                                      //realloc!!
                                                      //keep track of num of rwos 
                                                      //one while loop --> readLine until not null
                                                      //call reallaoc, expand it by 1 
                                                      //
    int row = 0;
    char *line;
    game -> board = NULL;
    while ((line = read_line(fp)) != NULL) {
        game -> board = realloc(game -> board, (row+1)*sizeof(char*)); //want to realloc by one more memory
        game -> board[row] = line;
        row++;
    }
...
static void find_head(game_t *game, unsigned int snum) {
  ...
  char bodycharacter = game -> board[row][column]; //checking it for head
    //printf("%c\n", c)

  while(is_snake(bodycharacter)) {
   //   printf("Current char at (%u, %u) is '%c'\n", row, column, c);
      if (is_head(bodycharacter)) {
          game ->snakes[snum].head_row = row;
          game ->snakes[snum].head_col = column;
          return;
      } 

      unsigned int next_row = get_next_row(row, bodycharacter);
      unsigned int next_column = get_next_col(column, bodycharacter);
          
      row = next_row;
      column = next_column;
      bodycharacter = game -> board[row][column];
  }
 return;
}
...
game_t *initialize_snakes(game_t *game) {
  // TODO: Implement this function.
  char **board_load = game -> board;
  //need to see the number of snakes present on the board
  //need to see the positions of the snakes on the board
  //start with 0 snakes and snake index of 0 as well
  unsigned int num_snakes = 0;
  unsigned int snake_count = 0;
  unsigned int num_rows = game -> num_rows;

  for (int i = 0; i < num_rows; i++) {
      for (int j = 0; j < strlen(board_load[i]); j++) {
            if(is_head(board_load[i][j])) {
                num_snakes++;
            }
        }
    }
    game -> num_snakes = num_snakes;
    game -> snakes = malloc(sizeof(snake_t) * num_snakes);

    for (int i =0; i < num_rows; i++) {
        for (int j =0; j<strlen(board_load[i]); j++) {
            if (is_tail(board_load[i][j])) {
                game -> snakes[snake_count].tail_row = i;
                game-> snakes[snake_count].tail_col = j;
